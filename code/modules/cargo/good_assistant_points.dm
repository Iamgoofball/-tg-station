...
/obj/item/gbp_punchcard/Initialize(mapload)
	. = ..()
	AddComponent(/datum/component/germ_sensitive, mapload)
	AddComponent(/datum/component/decomposition, mapload, decomp_req_handle = TRUE, decomp_flags = RAW, decomp_result = /obj/item/food/badrecipe/moldy, ant_attracting = FALSE, custom_time = 0, stink_particles = /particles/stink)
