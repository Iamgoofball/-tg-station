...
/datum/reagents/proc/add_reagent(
	...
	for(var/datum/reagent/iter_reagent as anything in cached_reagents)
		if(iter_reagent.type == reagent_type)
			...
			iter_reagent.on_merge(data, amount) // Update this before updating volume. FIXME: Move all of the surrounding poor quality into this proc so implementations decide whether they go first or base code goes first.
