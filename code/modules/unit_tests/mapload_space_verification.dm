...
/datum/unit_test/maptest_mapload_space_verification/Run()
	...
	for(var/turf/iterated_turf in ALL_TURFS())
		...
		if(HAS_TRAIT(iterated_turf, TRAIT_HYPERSPACE_STOPPED))
			continue // This means that a shuttle with a noop template turf is just temporarily parked ontop of us. That we're not actually a part of it.. We don't have to care about it as it will leave us alone when it flies away.
...
/datum/unit_test/maptest_mapload_space_verification/proc/validate_planetary_map()
	...
	testable_levels += SSmapping.levels_by_trait(ZTRAIT_STATION) // Station z-levels get to be in by default because they can derail an entire round. Cause LINDA to weep if a space turf is present.

	var/list/mining_levels = SSmapping.levels_by_trait(ZTRAIT_MINING)
	// Add in mining levels should they exist. Dupecheck to make sure we don't have any duplicates because it's valid to have a station. Mining level be the same.
	...
	for(var/level in testable_levels)
		...
		for(var/turf/open/space/iterated_turf in testable_turfs)
			// grab the type of the area that we're in too, because there's three different types of area types that have the name "Icemoon Wastes", f. Example
