...
/mob/living/basic/slime/verb/Feed()
	set category = "Slime"
...
/mob/living/basic/slime/proc/CanFeedon(mob/living/M, silent = FALSE)
	if(!Adjacent(M))
		return FALSE
	...
	if(locate(/mob/living/basic/slime) in M.buckled_mobs)
		if(silent)
			return FALSE
...
/mob/living/basic/slime/proc/Feedon(mob/living/M)
...
/mob/living/basic/slime/proc/Feedstop(silent = FALSE, living=1)
	if(buckled)
		if(!living)
			to_chat(src, "<span class='warning'>[pick("This subject is incompatible", \
			"This subject does not have life energy", "This subject is empty", \
...
/mob/living/basic/slime/verb/Evolve()
	set category = "Slime"
...
/mob/living/basic/slime/verb/Reproduce()
	set category = "Slime"
	...
	if(is_adult)
		if(amount_grown >= SLIME_EVOLUTION_THRESHOLD)
			...
			var/mob/living/basic/slime/new_slime = pick(babies)
