...
/mob/living/basic/slime/proc/handle_feeding(delta_time)
	if(!ismob(buckled))
		return
	var/mob/M = buckled
	...
	add_nutrition(7 * CONFIG_GET(number/damage_multiplier))

	//Heal yourself.
	adjustBruteLoss(-1.5)

/mob/living/basic/slime/proc/handle_nutrition(delta_time)
