...
/datum/bt_node/ai_behavior/capricious_retaliate/perform(seconds_per_tick, datum/ai_controller/controller)
	...
	if(controller.blackboard_key_exists(BB_BASIC_MOB_RETALIATE_LIST))
		...
		if(prob(deaggro_chance)) //Chance to chill the bad out. This prob() should be matched with the frequency of calling.
	...
	// Add to target_list  set_blackboard_key_assoc_lazylist calls post_blackboard_key_set, waking the combat branch
