 ...
 */
#define MINIMAP_FLAG_TGMC (1<<0)
#define MINIMAP_FLAG_CREW (1<<1)
#define MINIMAP_FLAG_SYNDIES (1<<2)
#define MINIMAP_FLAG_ABDUCTORS (1<<3)
...
/datum/controller/subsystem/minimaps/proc/load_new_z(datum/dcs, datum/space_level/z_level)
	...
	if(!is_station_level(level) && !is_mining_level(level) && !is_away_level(level))
		return
	var/icon/icon_gen = new('icons/ui_icons/antags/pirates/tgmc_minimap_template.dmi') //480x480 blank icon template for drawing on the map
	...
	for(var/datum/callback/callback in earlyadds["[level]"])
		callback.Invoke()
...
/datum/controller/subsystem/minimaps/proc/add_marker(atom/target, hud_flags = NONE, image/blip)
	...
	if(!initialized || !(minimaps_by_z["[target.z]"])) //the minimap doesn't exist yet, z level was probably loaded after init
		for(var/datum/callback/callback in earlyadds["[target.z]"])
	...
	for(var/flag in bitfield2list(hud_flags))
		...
		for(var/datum/minimap_updator/updator in update_targets["[flag]"])
...
/datum/controller/subsystem/minimaps/proc/get_drawing_image(zlevel, minimap_flag)
	...
	blip.icon = icon('icons/ui_icons/antags/pirates/tgmc_minimap_template.dmi')
...
/atom/movable/screen/minimap_locator
	name = "You are here"
	icon = 'icons/ui_icons/antags/pirates/tgmc_minimap_blips.dmi'
	icon_state = "locator"
...
/datum/action/minimap/GiveAction(mob/M)
	. = ..()
...
/datum/action/minimap/HideFrom(mob/M)
