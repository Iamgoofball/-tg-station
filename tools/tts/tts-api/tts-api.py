...
def text_to_speech_handler(endpoint, voice, text, filter_complex, pitch, special_filters = []):
	...
	current_ip = ips[current_ip_index]
	current_ip_index = (current_ip_index + 1) % len(ips)  # Cycle through the IPs
	if debug:
		print("Request Recieved: " +endpoint+ " " +voice+ " " +pitch+ " " +text+ " " + filter_complex + " " + "|".join(special_filters))
	filter_complex = filter_complex.replace("\"", "")
	hash2 = endpoint+voice+pitch+text+filter_complex + "|".join(special_filters)
	md5_sample = hash(hash2)
	cache_name = "cache_"+str(md5_sample)+".ogg"
	#if os.path.isfile("./cache/" + cache_name):
		#if debug:
		#	print("Sending cached file ./cache/" + cache_name)
		#data_bytes = io.BytesIO()
		#cached_file = pydub.AudioSegment.from_file("./cache/" + cache_name, "ogg")
		#length = cached_file.duration_seconds
		#cached_file.export(data_bytes, format="ogg")
		#export_audio = io.BytesIO(data_bytes.getvalue())
	...
	for sentence in segmenter.segment(text):
		path = current_ip
		response = requests.get(path + endpoint, json={ 'text': sentence, 'voice': voice, 'pitch': pitch })
	...
	finished_audio = pydub.AudioSegment.from_file(export_audio, "ogg")
	if debug:
		print("duration: " + str(finished_audio.duration_seconds))
	if finished_audio.duration_seconds > 26:
		print("BAD GENERATION: " +endpoint+ " " +voice+ " " +pitch+ " " +text+ " " + filter_complex + " " + "|".join(special_filters))
	length = finished_audio.duration_seconds
	new_data_bytes = io.BytesIO()
	finished_audio.export(new_data_bytes, format="ogg")
	export_audio = io.BytesIO(new_data_bytes.getvalue())
	#finished_audio.export("./cache/" + cache_name, format="ogg")
	response = send_file(export_audio, as_attachment=True, download_name='identifier.ogg', mimetype="audio/ogg")
	response.headers['audio-length'] = length
	return response
...
def text_to_speech_normal():
	...
	output = text_to_speech_handler("5003/generate-tts", voice, text, filter_complex, pitch, special_filters)
	response_times.append(time.time() - response_time)
	print("Average response time: " + str(sum(response_times) / len(response_times)))
	return output
...
def text_to_speech_blips():
	...
	filter_complex = request.args.get("filter", '')
	return text_to_speech_handler("5004/generate-tts-blips", voice, text, filter_complex, pitch, special_filters)
...
def voices_list():
	if authorization_token != request.headers.get("Authorization", ""):
		abort(401)
	if debug:
		print("Received request to handshake from " + str(request.remote_addr))
...
#		dummy_request = requests.get(f"http://127.0.0.1:5003/generate-tts", json={ 'text': "The quick brown fox jumps over the lazy dog.", 'voice': voice, 'pitch': str(0) })
#		if dummy_request.status_code != 200:
#			print(dummy_request.status_code)
#			print("FAILED TO DO PITCH " + str(0) + " VOICE " + str(voice))
#		dummy_request = requests.get(f"http://127.0.0.1:5003/generate-tts-blips", json={ 'text': "The quick brown fox jumps over the lazy dog.", 'voice': voice, 'pitch': str(0) })
#		if dummy_request.status_code != 200:
#			print(dummy_request.status_code)
#			print("FAILED TO DO BLIPS PITCH " + str(0) + " VOICE " + str(voice))
	response_times = [0]
	serve(app, host="0.0.0.0", port=5002, threads=6, backlog=40, connection_limit=200, channel_timeout=10)
